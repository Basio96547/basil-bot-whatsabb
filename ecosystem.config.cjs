// pm2 start ecosystem.config.cjs   (بند 8، طبقة 2: يرجّع العملية تلقائياً لو انهارت)
// pm2 save                        (يحفظ القائمة عشان pm2 resurrect تشتغل بعد إعادة الإقلاع)
module.exports = {
  apps: [
    {
      name: 'sms-api',
      script: 'src/index.ts',
      interpreter: 'node',
      // سقف كومة V8 مضبوط تحت max_memory_restart بهامش. بلا هذا، V8 يقدّر
      // الحد من ذاكرة الجهاز كلها (جيغابايتات) فيؤجّل الكنس، فتنمو الكومة
      // بهدوء حتى يقتلها pm2 عند ٤٠٠ ميغا — وكل قتلة تعني إقلاع Node وترجمة
      // TypeScript وإعادة ربط واتساب من الصفر، وهو أكبر مصدر حرارة في هذا
      // الإعداد. إخبار V8 بالحد يجعله يكنس قبل الاصطدام بدل أن يصطدم.
      interpreter_args: '--import tsx --max-old-space-size=256',
      cwd: __dirname,
      autorestart: true,
      restart_delay: 3000,
      // بند 9، نقطة 4 — سقف يمنع تسرّب ذاكرة من ابتلاع الجوال. كان 200M،
      // وقياس فعلي على الجهاز (2026-08-22) أظهر أن الاستهلاك الطبيعي بعد
      // تحميل جلسة Baileys يتجاوزه خلال دقيقتين من الإقلاع — فصار pm2 يقتل
      // الخدمة كل دقيقتين، وكل قتلة تعني إعادة اتصال واتساب من جديد. إعادة
      // اتصال متكررة على عميل غير رسمي هي بالضبط ما يرفع خطر حظر الرقم، أي
      // أن الحد كان يصنع الضرر الذي وُضع ليمنعه. 400M يترك هامشاً للتشغيل
      // الطبيعي ويظل يمسك أي تسرّب حقيقي.
      max_memory_restart: '400M',
      env: { NODE_ENV: 'production' },
    },
    {
      // بدون هذا التونيل، ووركر المتجر ما يقدر يوصل للخدمة أصلاً — الجوال
      // عادةً بلا IP عام (خصوصاً على نت الموبايل). راجع README قسم الشبكة.
      name: 'cloudflared-tunnel',
      script: 'cloudflared',
      // --ha-connections 2 (كان 1): سجلات النفق أظهرت أن شبكة هذا الجوال
      // تفقد نشاطها بشكل متكرر ("timeout: no recent network activity")،
      // وباتصال QUIC واحد فإن كل انقطاع كهذا يُسقط النفق كاملاً لثوانٍ — أي
      // انقطاع فعلي لكل الطلبات الواردة (sms-api وbot.saudiarabiafireworkstore
      // معاً عبر نفس النفق)، لا فقط إرسال واتساب. اتصالان يعنيان أن انقطاع
      // أحدهما وحده لا يُسقط النفق، بتكلفة إيقاظ راديو إضافي طفيف — قرار
      // مقصود لصالح الصمود، بعد أن كان أربعة أضعاف هذا العدد (راجع تاريخ
      // الملف) خُفّض إلى واحد توفيراً للبطارية.
      // --no-autoupdate: يمنع فحص التحديث الدوري، ويمنع أسوأ منه — إعادة
      // تشغيل ذاتية للتونيل في منتصف الليل على جهاز لا أحد يراقبه.
      args: 'tunnel --no-autoupdate --ha-connections 2 run sms-api',
      autorestart: true,
      restart_delay: 3000,
    },
  ],
};
